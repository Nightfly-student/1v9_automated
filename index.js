...
app.post("/check", async (req, res) => {
  clipboard.setContent(req.body.user);
  const region = req.body.region.toUpperCase();

  const window = await findSLCReborn();

  await moveToFile(window);

  await sleep(500);

  await moveToClipboard(window);

  await sleep(500);

  await regionSelect(region);
  ...
  return res.send(csvJSON(await clipboard.getContent())[0])
});
